...
//Aug
__device__ int* gMatrix = nullptr;   

#define Matrix(k) (gMatrix[k])
...
__device__ int2 getGapOpenGapExtend(int DNAFlag, int matAvgScore, PairScaleValues scaleValues,
    ...
    if (DNAFlag){
        return make_int2( static_cast<int>(2 * pwGapOpen * intScale * gapOpenScale),
                          static_cast<int>(pwGapExtend * intScale * gapExtendScale));
    }
    else {
        int gapOpenVal = 0;
        if (matAvgScore <= 0)
            gapOpenVal = 2 * static_cast<int>((pwGapOpen + __logf(static_cast<float>(min(n,m)))) * intScale);
        else
            gapOpenVal = static_cast<int>(2 * matAvgScore * (pwGapOpen + __logf(static_cast<float>(min(n,m)))) * gapOpenScale);
...
__device__ int cudaDiff(unsigned char* sequence1, unsigned char* sequence2,
    ...
    int hh = 0;
    if(threadId == SPThread){
        diffArgv initArgv = {ScoreAndPosSb.y - 1, ScoreAndPosSb.z - 1,
                             ScoreAndPosSe.y - ScoreAndPosSb.y + 1,
                             ScoreAndPosSe.z - ScoreAndPosSb.z + 1, 0, 0, 0};
        StackInit(&stack);
                                             ...
                                             int2* element)
{
    ...
    int& gapOpen = gapOpen_gapExtend.x; 
    int& gapExtend = gapOpen_gapExtend.y; 
    int len1 = 0;
    int len2 = 0;
    ...
    if(threadId == SPThread){
        ...
        gapOpen_gapExtend = getGapOpenGapExtend(gpair.DNAFlag, gpair.matAvgScore,
                                                gpair.scaleValues, gpair.pwGapOpen, gpair.pwGapExtend,
                                                m, n);
    }
    __syncthreads();
    if(control)    return;
    ScoreAndPosSe = cudaForwardPath(sequence1, sequence2, n, m, gapOpen, gapExtend,
                                    &HH->array2D[indexHDRS], &DD->array2D[indexHDRS]); 
    ScoreAndPosSb = cudaReversePath(sequence1, sequence2, n, m, gapOpen, gapExtend,
                                    &HH->array2D[indexHDRS], &DD->array2D[indexHDRS], ScoreAndPosSe); 
    int printPtr = cudaDiff(sequence1, sequence2, &HH->array2D[indexHDRS], &DD->array2D[indexHDRS],
                            &RR->array2D[indexHDRS], &SS->array2D[indexHDRS],
                            &displ->array2D[indexDispl], gapOpen, gapExtend,
                            ScoreAndPosSe, ScoreAndPosSb);
    if(threadId == SPThread){
        double mmScore = (double)cudaTracePath(sequence1, sequence2, ScoreAndPosSb.y, ScoreAndPosSb.z,
                                               printPtr, &displ->array2D[indexDispl],
                                               gpair.gapPos1, gpair.gapPos2);
...
void cudaFullPairwiseAlign(Alignment* alignPtr, DistMatrix* distMat,
    ...
    #pragma omp parallel
    {
        ...
        //Aug
        int* tempGMatrix;
        cudaMalloc((void**)&tempGMatrix, NUMRES * NUMRES * sizeof(int));
        cudaMemcpy(tempGMatrix, matrix, NUMRES * NUMRES * sizeof(int),
                   cudaMemcpyHostToDevice);
        setGlobalMatrixPointer<<<1,1>>>(tempGMatrix);
        cudaDeviceSynchronize(); //Aug
        ...
        float* distMatrixHost;
        cudaMallocHost((void**)&distMatrixHost, sizeof(float)*numberOfBlocks*nstreams); 
        memset(distMatrixHost, 0, sizeof(float)*numberOfBlocks*nstreams);
        float* gDistMatrix;
        cudaMalloc((void**)&gDistMatrix, sizeof(float)*numberOfBlocks*nstreams);
        ...
        int2* gElement;
        cudaMalloc((void**)&gElement, sizeof(int2)*element_size );
        cudaMemcpy(gElement, element, sizeof(int2)*element_size, cudaMemcpyHostToDevice);
        ...
        for(int i = 0; i<element_size ; i+= numberOfStreamBlocks){
            ...
            for(int k = 0 ; k < nstreams ; k++){
                if(rest2 >= numberOfBlocks){
                    cudaMemcpyAsync(&distMatrixHost[k*numberOfBlocks],
                                    &gDistMatrix[k*numberOfBlocks],
            ...
            for(int k = 0; k<nstreams ; k++){
                ...
                for(int j=i+k*numberOfBlocks ; j<(i+(k+1)*numberOfBlocks) && j<element_size ; j++){
                    distMat->SetAt(element[j].x + 1, element[j].y + 1, distMatrixHost[j - i]);
                    distMat->SetAt(element[j].y + 1, element[j].x + 1, distMatrixHost[j - i]);
                    if(userParameters->getDisplayInfo()){
                        utilityObject->info("Sequences (%d:%d) Aligned. Score:  %d",
                                            element[j].x + 1, element[j].y+1,
                                            (int)(100.f - distMatrixHost[j - i]*100.f));
                    }
                }
            }
        }
